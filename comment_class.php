...
// ------------- TODO move the 'listing' into separate function so that ajax can access it easily. 
	...
	function nextprev($table, $id, $from=0)
	{
		...
		{
			return "<a class='e-loadmore btn btn-primary btn-sm' 
						href='#' 
						data-src='".e_HTTP . "comment.php?mode=list&type={$table}&id={$id}&from={$this->commentsPerPage}#comments'
						data-from='{$this->commentsPerPage}' 
						data-inc='{$this->commentsPerPage}' 
						data-total='{$this->totalComments}'
						data-target='#comments-container-".e107::getForm()->name2id($table)."'>
						". LAN_MORE_COMMENTS . "
					</a>";
		}
